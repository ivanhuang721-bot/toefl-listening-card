const CACHE='toefl-listening-card-v10';
const ASSETS=['./','./index.html','./manifest.webmanifest','./sw.js'];

function transformIndex(html){
  html=html.replace('v7 · 驾驶模式支持原句开关','v10 · 听力熟悉度 + 继续上次');
...
`;
  ...
  html=html.replace("document.querySelector('#blindToggle').onclick=()=>{setToggleState('blindToggle','blind',!driver.blind);driver.wordShown=!driver.blind;document.querySelector('#driverWordToggle').textContent=driver.wordShown?'🙈 隐藏单词':'👁 显示单词'};", "document.querySelector('#blindToggle').onclick=()=>{setToggleState('blindToggle','blind',!driver.blind);driver.wordShown=!driver.blind;document.querySelector('#driverWordToggle').textContent=driver.wordShown?'🙈 隐藏单词':'👁 显示单词'};");
  html=html.replace("document.querySelector('#driverStart').onclick=openDriver;document.querySelector('#openDriver').onclick=openDriver;document.querySelector('#driverStop').onclick=stopDriver;", "document.querySelector('#driverStart').onclick=openDriver;document.querySelector('#openDriver').onclick=openDriver;document.querySelector('#driverStop').onclick=stopDriver;");
  return html;
}
